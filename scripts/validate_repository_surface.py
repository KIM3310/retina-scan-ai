...
PRIVATE_INQUIRY_URL = (
    ...
    "?offer=retina-scan-ai&inquiry=industrial-validation-discovery#private-inquiry"
)
DISCOVERY_PHRASES = (
    "non-clinical",
    "synthetic",
...
def check_service_offer(path: Path) -> None:
    ...
    if not isinstance(offers, list) or len(offers) < 2:
        fail(
            f"{path.relative_to(ROOT)} structured_data.offers must include free and private offers"
        )
    paid_offer = offers[1]
